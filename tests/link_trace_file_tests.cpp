...
int main() {
    ...
    if (trace.is_open()) {
        ...
        trace.write_event("transport_state", "state=connected peer_ready=1");
        ...
        check(contents.find(
                  "event=transport_state trace_version=1 session_id=1 frame=0 elapsed_ms=") !=
                  std::string::npos,
              "SDL trace lifecycle events use the canonical event prefix");
        check(contents.find("transport=tcp role=host state=connected peer_ready=1") !=
                  std::string::npos,
              "SDL trace lifecycle events preserve structured fields");
