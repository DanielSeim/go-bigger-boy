...
def load_manifest(path: Path) -> dict[str, object]:
    ...
    if reference is not None:
        ...
        checkpoints = "checkpoints" in reference
        full_sequence = "sequence" in reference
        digest_only = "frame_sha256" in reference
        if full_sequence:
            if any(key in reference for key in ("checkpoints", "frame_sha256", "image",
                                                 "image_sha256", "region", "channel_tolerance",
                                                 "max_mismatched_pixels")):
                raise ValueError("sequence reference cannot include other image fields")
        ...
        if checkpoints:
            ...
            checkpoint_list = reference["checkpoints"]
            if not isinstance(checkpoint_list, list) or not 2 <= len(checkpoint_list) <= 64:
            ...
            if previous_frame != manifest["frames"] or previous_frame - checkpoint_list[0]["frame"] > 1000:
        ...
        if not checkpoints and not full_sequence:
...
def validate(manifest_path: Path, rom: Path, runner: Path,
    ...
    reference = manifest.get("reference")
    checkpoints = isinstance(reference, dict) and "checkpoints" in reference
    full_sequence = isinstance(reference, dict) and "sequence" in reference
    sequence = checkpoints or full_sequence
    reference_hashes = None
    if full_sequence:
        details = reference["sequence"]
        hashes_path = Path(details["hashes"])
        if not hashes_path.is_absolute():
            hashes_path = manifest_path.resolve().parent / hashes_path
        if file_digest(hashes_path) != details["hashes_sha256"]:
            raise ValueError("reference sequence hashes SHA-256 does not match the manifest")
        reference_hashes = hashes_path.read_text(encoding="ascii").splitlines()
        if len(reference_hashes) != manifest["frames"] - details["first_frame"] + 1:
            raise ValueError("reference sequence has the wrong number of frame hashes")
        if any(not SHA256.fullmatch(digest) for digest in reference_hashes):
            raise ValueError("reference sequence contains an invalid frame hash")
    ...
    if isinstance(reference, dict):
        if checkpoints:
            ...
            for index, expected in enumerate(reference_hashes):
                frame = first + index
                path = output_dir / f"sgb-frame-{frame}.ppm"
                actual = file_digest(path)
                if actual == expected:
                    matched += 1
    ...
    report = {
        ...
        "reference": None if comparison is None else {
            ...
            **({"checkpoints": reference["checkpoints"]} if checkpoints else
               {"sequence": reference["sequence"]} if full_sequence else
